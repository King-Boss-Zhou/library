...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>借书页面</title>
        <link rel="stylesheet" type="text/css" href="borrow/borrow.css"/>
    </head>
    ...
    <body>
        ...
        <div id="Borrow">
            ...
            <form method="post" id="Condition">
                ...
                <div id="borrow_control">
                    <input type="submit" id="search_button" name="search_button" value="查询"/>
                    <input type="submit" id="borrow_button" name="borrow_button" value="借阅"/>
                </div>
            </form>
        </div>
        ...
        <?php
            if(isset($_POST['borrow_button']) && $_POST['borrow_button'] == "借阅")
            {
                ...
                if($conn->connect_error)    //Data base connet failed
                {
                    ...
                    echo "<script>location='book_borrow.php';</script>";
                }
                else
                {
                    ...
                    if($stmt->num_rows)
                    {
                        $sql = "SELECT store FROM book WHERE book_ID = ?";
                        $stmt = $conn->prepare($sql);
                        ...
                        if($stmt->num_rows)
                        {
                            if($store >= 1)
                            {
                                $sql = "INSERT INTO record (book_ID, card_ID, borrow_time, return_time, manager_ID) VALUES(?,?,?,NULL,?)";
                                $stmt = $conn->prepare($sql);
                                $stmt->bind_param("ssss", $book_ID, $card_ID, $time, $_SESSION['username']);
                                $stmt->execute();
                                ...
                                echo "<script language=\"JavaScript\">alert(\"Borrow successfully!\");</script>";
                                echo "<script>location='book_borrow.php';</script>";
                            }
                            else
                            {
                                echo "<script language=\"JavaScript\">alert(\"Stored number is 0!\");</script>";
                                echo "<script>location='book_borrow.php';</script>";
                            }
                        }
                        else
